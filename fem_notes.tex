...
\begin{figure}[h]
    \centering
    \includegraphics[width=\textwidth]{./analysis/conv_beam.pdf}
...
L'analyse de convergence du code a été réalisée sur un cylindre creux de
rayon interne $r_1=10$ cm, de rayon externe $r_2=15$ cm, de hauteur $H=20$ cm,
avec $E=40$ GPa, $\rho=2300$ kg/m$^3$, $\nu=0.2$, soumis à une pression interne 
de $5$ kPa et à une pression externe de $1$ kPa.
L'analyse est présentée à la figure \ref{fig:convTank}.

Dans le cas plane, le vecteur déplacement $\uu$ et le tenseur des contraintes 
$\stress$ sont transformées des coordonées cartésiennes en coordonnées 
cylindriques. L'erreur $L_2$ est ensuite calculée sur base de la solution
analytique :
\begin{align}
    \textrm{e}^2(\uu^h) &= 
    \frac
        {\displaystyle \int_{\Omega} \left\|\uu^h - \uu^*\right\|^2 \:\dd \xx}
        {\displaystyle \int_{\Omega} \left\|\uu^*\right\|^2 \:\dd \xx}
    = \frac
        {\displaystyle \int_{\Omega} \left(u_r^h - u_r^*\right)^2 \:\dd \xx}
        {\displaystyle \int_{\Omega} \left(u_r^*\right)^2 \:\dd \xx}\\
    \textrm{e}^2(\sigma_i^h) &=  
    \frac
        {\displaystyle \int_{\Omega} \left(\sigma_i^h - \sigma_i^*\right)^2 \:\dd \xx}
        {\displaystyle \int_{\Omega} \left(\sigma_i^*\right)^2 \:\dd \xx}
    \quad \text{avec } i \in \{rr, \theta\theta, zz\}\\
\end{align}

On constate que les modèles 2D et 3D convergent à la même vitesse vers la 
solution, mais que le modèle 3D présente tout de même une erreur légèrement 
plus faible pour une taille de maille similaire. 

On constate également que le taux de convergence pour les contraintes est 
superlinéaire quel que soit la méthode de calcul utilisée 
(moindres carrés ou moyenne pondérée, cf. section \ref{sec:contraintes}
pour plus de détails).

\begin{figure}[h]
    \centering
    \includegraphics[width=\textwidth]{./analysis/conv_tank.pdf}
    \caption{Analyse de convergence pour un cylindre creux.}
    \label{fig:convTank}
\end{figure}
