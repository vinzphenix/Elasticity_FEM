...
l'itération de la puissance converge uniquement vers le mode propre $\Phi_i$ 
associé à
\begin{equation}
...
\begin{alignat}{2}
    ...
    &\implies &\qquad x_{k} &= (K^{-1} M) x_{0} = \sum_{j=1}^{n} \alpha_j (K^{-1} M)^{k} \Phi_j
...
initial $x_0$ ne contient pas de composante $\Phi_1$. Lorsque ce premier mode
propre est trouvé  il faut donc le "supprimer" d'un itéré $w$ pour 
...
le spectre du système pour trouver le mode propre autout d'une fréquence cible.
En effet, $K-\mu M$ possède les mêmes vecteurs propres $\Phi_i$ que $K$ mais 
avec des valeurs propres décalées $\lambda_i - \mu$ :
\begin{equation}
...
L'itération inverse
\begin{equation}
    x_{k+1} \gets (K - \mu M)^{-1} M x_k \iff
    \left\{\begin{aligned}
        z_{k} &\gets M x_k\\
        (K - \mu M) x_{k+1} &\gets z_k
    \end{aligned}\right\}
\end{equation}
